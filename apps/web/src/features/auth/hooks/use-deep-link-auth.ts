  ...
  useEffect(() => {
    ...
    function processDeepLink(url: string) {
      try {
        ...
        // Our custom scheme is configured as kbm://
        // Redirects will come as kbm://auth/callback?code=...
        if (parsedUrl.host === 'auth' && parsedUrl.pathname.includes('callback')) {
          ...
          const targetUrl = window.location.origin + '/' + parsedUrl.host + parsedUrl.pathname + parsedUrl.search + parsedUrl.hash;
